    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if abstract_inverted_index:
                 return reconstruct_openalex_abstract(abstract_inverted_index)
            # else:
            #      print(f"    ⚠️ OpenAlex: No abstract found for {doi}")
        else:
...
def fetch_abstract_from_url(url, retries=2):
    ...
    try:
        for attempt in range(retries):
            ...
            if found_abstract:
                return found_abstract, image_url
            
            # If we got image but not abstract, we can still return partial result if we are desperate? 
            # But the function signature implies we return None if failed. 
            # However, caller expects abstract mostly.
            # Let's return None for abstract, but Image? The caller logic relies on abstract presence to proceed often.
...
def export_to_siyuan(title, content, score, notebook_id=None):
    ...
    path = f"/EssayUpdate/KeyUpdate/{curr_date}/{safe_title}"
...
def main():
    ...
    for journal_name, rss_url in RSS_JOURNALS.items():
        ...
        try:
            ...
            for entry in feed.entries:
                ...
                fetched_abstract, fetched_image = fetch_abstract_from_url(link)
    ...
    for journal_name, issn in CROSSREF_JOURNALS.items():
        ...
        for entry in entries:
            ...
            full_abstract, fetched_image = fetch_abstract_from_url(entry.link)
            
            # 2. OpenAlex (Preferred Metdata Source)
            if not full_abstract and hasattr(entry, 'doi') and entry.doi:
                 full_abstract = fetch_metadata_from_openalex(entry.doi)
